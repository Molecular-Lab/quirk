...
export const getVaultStrategyQuery = `-- name: GetVaultStrategy :one

SELECT id, client_vault_id, category, target_percent, created_at, updated_at FROM vault_strategies
WHERE id = $1 LIMIT 1`;

export interface GetVaultStrategyArgs {
    id: string;
}

export interface GetVaultStrategyRow {
    id: string;
...
export async function getVaultStrategy(sql: Sql, args: GetVaultStrategyArgs): Promise<GetVaultStrategyRow | null> {
    const rows = await sql.unsafe(getVaultStrategyQuery, [args.id]).values();
    if (rows.length !== 1) {
        return null;
    }
    const row = rows[0];
...
export const getVaultStrategyByCategoryQuery = `-- name: GetVaultStrategyByCategory :one
SELECT id, client_vault_id, category, target_percent, created_at, updated_at FROM vault_strategies
WHERE client_vault_id = $1
  AND category = $2
LIMIT 1`;

export interface GetVaultStrategyByCategoryArgs {
    clientVaultId: string;
    category: string;
}

export interface GetVaultStrategyByCategoryRow {
    id: string;
    clientVaultId: string;
    category: string;
    targetPercent: string;
...
export async function getVaultStrategyByCategory(sql: Sql, args: GetVaultStrategyByCategoryArgs): Promise<GetVaultStrategyByCategoryRow | null> {
    const rows = await sql.unsafe(getVaultStrategyByCategoryQuery, [args.clientVaultId, args.category]).values();
    if (rows.length !== 1) {
        return null;
    }
    const row = rows[0];
...
export const listVaultStrategiesQuery = `-- name: ListVaultStrategies :many
SELECT id, client_vault_id, category, target_percent, created_at, updated_at FROM vault_strategies
WHERE client_vault_id = $1
ORDER BY target_percent DESC`;

export interface ListVaultStrategiesArgs {
    clientVaultId: string;
}

export interface ListVaultStrategiesRow {
    id: string;
    clientVaultId: string;
    category: string;
    targetPercent: string;
...
export async function listVaultStrategies(sql: Sql, args: ListVaultStrategiesArgs): Promise<ListVaultStrategiesRow[]> {
    return (await sql.unsafe(listVaultStrategiesQuery, [args.clientVaultId]).values()).map(row => ({
        id: row[0],
        clientVaultId: row[1],
        category: row[2],
        targetPercent: row[3],
...
export const updateVaultStrategyQuery = `-- name: UpdateVaultStrategy :one
...
export interface UpdateVaultStrategyArgs {
    id: string;
    targetPercent: string;
}

export interface UpdateVaultStrategyRow {
    id: string;
    clientVaultId: string;
    category: string;
    targetPercent: string;
...
export async function updateVaultStrategy(sql: Sql, args: UpdateVaultStrategyArgs): Promise<UpdateVaultStrategyRow | null> {
    const rows = await sql.unsafe(updateVaultStrategyQuery, [args.id, args.targetPercent]).values();
    if (rows.length !== 1) {
        return null;
    }
    const row = rows[0];
...
export const deleteVaultStrategyQuery = `-- name: DeleteVaultStrategy :exec
DELETE FROM vault_strategies
WHERE id = $1`;

export interface DeleteVaultStrategyArgs {
    id: string;
}

export async function deleteVaultStrategy(sql: Sql, args: DeleteVaultStrategyArgs): Promise<void> {
    await sql.unsafe(deleteVaultStrategyQuery, [args.id]);
}

export const deleteVaultStrategiesByVaultQuery = `-- name: DeleteVaultStrategiesByVault :exec
DELETE FROM vault_strategies
WHERE client_vault_id = $1`;

export interface DeleteVaultStrategiesByVaultArgs {
    clientVaultId: string;
}

export async function deleteVaultStrategiesByVault(sql: Sql, args: DeleteVaultStrategiesByVaultArgs): Promise<void> {
    await sql.unsafe(deleteVaultStrategiesByVaultQuery, [args.clientVaultId]);
}
