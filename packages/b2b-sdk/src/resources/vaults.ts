...
import type { CreateVaultRequest, MarkStakedRequest, UpdateVaultIndexRequest, Vault } from "../types"
...
export class VaultResource {
	 ...
	 */
	async getByToken(clientId: string, tokenSymbol: string, chainId: number): Promise<Vault> {
		return this.http.get<Vault>(`/api/v1/vaults/token/${clientId}/${tokenSymbol}/${chainId}`)
	}
	 ...
	 */
	async markStaked(vaultId: string, data: MarkStakedRequest): Promise<{ success: boolean; message: string }> {
