...
export class WithdrawalResource {
  ...
  async listByClient(
    ...
    return this.http.get<Withdrawal[]>(
      `/api/v1/withdrawals/client/${clientId}${queryString}`
    )
  }
  ...
  async listByUser(
    ...
    const queryString = params ? this.http.buildQueryString(params) : ''
    return this.http.get<Withdrawal[]>(
      `/api/v1/withdrawals/user/${userId}${queryString}`
    )
  }
  ...
  async getStats(
    ...
    const queryString = params ? this.http.buildQueryString(params) : ''
    return this.http.get<WithdrawalStats>(
      `/api/v1/withdrawals/stats/${clientId}${queryString}`
    )
  }
}
