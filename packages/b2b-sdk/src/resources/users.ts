...
export class UserResource {
	 ...
	 */
	async getByClientUserId(clientId: string, clientUserId: string): Promise<User> {
		return this.http.get<User>(`/api/v1/users/client/${clientId}/user/${clientUserId}`)
	}
	 ...
	 */
	async listByClient(clientId: string, params?: PaginationParams): Promise<User[]> {
	 ...
	 */
	async getBalance(userId: string, params?: { chain?: string; token?: string }): Promise<UserBalance> {
		const queryString = params ? this.http.buildQueryString(params) : ""
		return this.http.get<UserBalance>(`/api/v1/users/${userId}/balance${queryString}`)
	}
