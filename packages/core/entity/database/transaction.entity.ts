...
export const depositTransactionSchema = z.object({
  ...
  // Fees
  gatewayFee: z.string().regex(/^\d+(\.\d+)?$/).nullable(),
  proxifyFee: z.string().regex(/^\d+(\.\d+)?$/).nullable(),
  networkFee: z.string().regex(/^\d+(\.\d+)?$/).nullable(),
  totalFees: z.string().regex(/^\d+(\.\d+)?$/).nullable(),
...
export const withdrawalTransactionSchema = z.object({
  ...
  actualAmount: z.string().regex(/^\d+(\.\d+)?$/).nullable(),
  currency: z.string().length(3),
  ...
  // Fees
  withdrawalFee: z.string().regex(/^\d+(\.\d+)?$/).nullable(),
  networkFee: z.string().regex(/^\d+(\.\d+)?$/).nullable(),
  totalFees: z.string().regex(/^\d+(\.\d+)?$/).nullable(),
