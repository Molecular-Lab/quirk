...
export const clientVaultSchema = z.object({
	...
	idleBalance: z
		.string()
		.regex(/^\d+(\.\d+)?$/)
		.default("0"), // Funds ready to stake
	earningBalance: z
		.string()
		.regex(/^\d+(\.\d+)?$/)
		.default("0"), // Funds actively earning

	// Revenue Tracking (NEW - 3-way split)
	clientRevenueEarned: z
		.string()
		.regex(/^\d+(\.\d+)?$/)
		.default("0"), // Client's share (10-20%)
	platformRevenueEarned: z
		.string()
		.regex(/^\d+(\.\d+)?$/)
		.default("0"), // Platform's share (5-10%)
	enduserRevenueEarned: z
		.string()
		.regex(/^\d+(\.\d+)?$/)
		.default("0"), // End-users' share (remainder)

	// Deprecated (kept for backward compatibility)
	pendingDepositBalance: z
		.string()
		.regex(/^\d+(\.\d+)?$/)
		.optional(), // Use idleBalance instead
	totalStakedBalance: z
		.string()
		.regex(/^\d+(\.\d+)?$/)
		.optional(), // Use earningBalance instead
	cumulativeYield: z
		.string()
		.regex(/^\d+(\.\d+)?$/)
		.optional(), // Raw yield before splits
...
export const createClientVaultSchema = z.object({
	...
	currentIndex: z
		.string()
		.regex(/^\d+(\.\d+)?$/, "Invalid index format")
		.default("1.0"),
	totalShares: z
		.string()
		.regex(/^\d+(\.\d+)?$/, "Invalid shares format")
		.default("0"),
	totalDeposited: z
		.string()
