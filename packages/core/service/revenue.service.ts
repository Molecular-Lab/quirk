...
const Logger = {
  info: (msg: string, meta?: any) => console.log(`[RevenueService] ${msg}`, meta || ''),
  error: (msg: string, meta?: any) => console.error(`[RevenueService] ${msg}`, meta || ''),
}
...
export class RevenueService {
  ...
  async calculateMRR(clientId: string): Promise<MRRCalculationResult> {
    try {
      ...
      const mrr = this.deps.clientRepository.calculateMRR(
        earningBalance,
        avgApy30d,
  ...
  async distributeYield(vaultId: string, rawYield: string): Promise<YieldDistribution> {
    try {
      ...
      if (Math.abs(total - rawYieldNum) > tolerance) {
        throw new Error(
          `Revenue split validation failed: ${total.toFixed(6)} !== ${rawYield}`
        )
      }
