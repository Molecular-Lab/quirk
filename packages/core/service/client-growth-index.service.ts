...
export class ClientGrowthIndexService {
  ...
  async calculateClientGrowthIndex(clientId: string): Promise<string> {
    ...
    const clientGrowthIndex = weightedIndexSum
      .dividedBy(totalAUM)
  ...
  async getClientGrowthIndexBreakdown(clientId: string) {
    ...
    const clientGrowthIndexDecimal = new BigNumber(clientGrowthIndex)
      .dividedBy('1000000000000000000')
      .toString();

    const totalAUM = breakdown.reduce(
      (sum, v) => sum.plus(v.aum),
      new BigNumber(0)
    );
