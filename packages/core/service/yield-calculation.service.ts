...
export class YieldCalculationService {
  ...
  static calculateWeightedAPY(
    ...
    for (const strategy of strategies) {
      ...
      console.log(`[YieldCalc] ${strategy.protocol}: ${strategy.allocation}% allocation * ${protocolData.apy}% APY = ${contribution.toFixed(4)}% contribution`);
    }
  ...
  static calculateNewIndex(params: {
    ...
    if (newIndex.greaterThan(maxDailyGrowth)) {
      throw new Error(`[YieldCalc] Index growth too large! Old: ${currentIndexDecimal.toString()}, New: ${newIndex.toString()}`);
    }
   ...
   */
  static calculateWithdrawalImpact(params: {
    totalStaked: string;
