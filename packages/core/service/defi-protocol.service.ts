...
export class DeFiProtocolService {
  ...
  constructor(chainId: number = 8453) { // Default: Base mainnet
   ...
   */
  async fetchAAVEMetrics(token: string = 'USDC', chainId: number = 8453): Promise<ProtocolData> {
    try {
   ...
   */
  async fetchCompoundMetrics(token: string = 'USDC', chainId: number = 8453): Promise<ProtocolData> {
    try {
      const [supplyAPY, borrowAPY, metrics] = await Promise.all([
   ...
   */
  async fetchMorphoMetrics(token: string = 'USDC', chainId: number = 8453): Promise<ProtocolData> {
    try {
   ...
   */
  async aggregateAllProtocols(token: string = 'USDC', chainId: number = 8453) {
