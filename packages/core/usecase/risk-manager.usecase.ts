...
export class RiskManagerUseCase implements IRiskManagerDataGateway {
	...
	async setRiskProfile(params: { walletAddress: string; profile: CreateRiskProfile }): Promise<RiskProfile> {
	...
	async updateRiskProfile(params: { walletAddress: string; updates: UpdateRiskProfile }): Promise<RiskProfile> {
	...
	async calculateRiskScore(params: { positions: DeFiPosition[]; riskProfile: RiskProfile }): Promise<RiskScore> {
	...
	async validateStrategy(params: { strategy: YieldStrategy; riskProfile: RiskProfile }): Promise<{
		isValid: boolean
		reasons: string[]
	...
	async getRiskRecommendations(params: { positions: DeFiPosition[]; riskProfile: RiskProfile }): Promise<{
	...
	async checkRiskLimits(params: { positions: DeFiPosition[]; riskProfile: RiskProfile }): Promise<{
