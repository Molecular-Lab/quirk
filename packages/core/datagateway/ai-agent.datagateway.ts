...
export interface IAIAgentDataGateway {
	 ...
	 */
	analyzeMarketConditions(params: { chainIds?: number[]; protocols?: string[] }): Promise<{
		timestamp: Date
		overall: {
	 ...
	 */
	executeStrategy(params: { walletAddress: string; strategyId: string; amount: string; chainId: number }): Promise<{
		transactions: {
			to: string
			data: string
			value: string
	 ...
	 */
	monitorPositions(params: { walletAddress: string; positions: DeFiPosition[] }): Promise<{
		alerts: {
	 ...
	 */
	analyzeSentiment(params: { protocol: string; timeRange?: "1h" | "24h" | "7d" | "30d" }): Promise<{
		protocol: string
