...
export class WithdrawalRepository {
	...
	async listByClient(clientId: string, limit = 100, offset = 0): Promise<ListWithdrawalsRow[]> {
		return await listWithdrawals(this.sql, { clientId, limit: limit.toString(), offset: offset.toString() })
	}

	async listByUser(clientId: string, userId: string, limit = 100): Promise<ListWithdrawalsByUserRow[]> {
		return await listWithdrawalsByUser(this.sql, { clientId, userId, limit: limit.toString(), offset: "0" })
	}

	async listByStatus(clientId: string, status: string, limit = 100): Promise<ListWithdrawalsByStatusRow[]> {
		return await listWithdrawalsByStatus(this.sql, { clientId, status, limit: limit.toString(), offset: "0" })
	}

	async listQueued(limit = 100): Promise<ListQueuedWithdrawalsRow[]> {
