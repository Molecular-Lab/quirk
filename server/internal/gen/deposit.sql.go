...
func (q *Queries) ListPendingDeposits(ctx context.Context, clientID uuid.UUID) ([]DepositTransaction, error) {
	rows, err := q.db.Query(ctx, listPendingDeposits, clientID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
