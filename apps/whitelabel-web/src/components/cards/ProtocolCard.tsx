...
export function ProtocolCard({
	...
	return (
		<div className={cn("rounded-lg border bg-white p-6 shadow-sm", !isActive && "opacity-60", className)}>
			<div className="flex items-start justify-between">
				<div>
					<div className="flex items-center gap-2">
						<span className={cn("rounded px-2 py-1 text-xs font-medium uppercase border", protocolColors[protocol])}>
							{name}
						</span>
						<span className={cn("rounded px-2 py-1 text-xs font-medium", riskColors[riskLevel])}>{riskLevel} risk</span>
					</div>
					{!isActive && <span className="mt-2 inline-block text-xs text-gray-500">Inactive</span>}
				</div>
			...
			<div className="mt-4 grid grid-cols-2 gap-4">
				<div>
					...
					<p className="mt-1 text-sm font-semibold text-gray-900">${amountDeployed.toLocaleString()}</p>
				</div>
				<div>
					<p className="text-xs text-gray-500">Current Value</p>
					<p className="mt-1 text-sm font-semibold text-gray-900">${currentValue.toLocaleString()}</p>
				</div>
				<div>
				...
				<div>
					...
					<p className={cn("mt-1 text-sm font-semibold", profit >= 0 ? "text-green-600" : "text-red-600")}>
