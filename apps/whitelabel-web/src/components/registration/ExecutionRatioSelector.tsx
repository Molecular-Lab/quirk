...
export default function ExecutionRatioSelector({
  ratio,
  onChange,
}: ExecutionRatioSelectorProps) {
  ...
  return (
    <div className="bg-white rounded-lg p-8 border border-gray-200">
      <h3 className="text-xl font-bold text-gray-900 mb-2">
        Execution Ratio Configuration
      </h3>
      <p className="text-gray-600 mb-8">
        Customize how your capital is distributed across protocols
      </p>
          ...
          {PROTOCOLS.map((protocol) => (
            <div key={protocol.key} className="space-y-2">
              ...
              <div className="text-xs text-gray-500">
                {protocol.key === 'aave' &&
                  'Stable lending protocol with proven track record'}
                {protocol.key === 'uniswap' &&
                  'DEX liquidity provision with dynamic fees'}
                {protocol.key === 'compound' &&
                  'Money market protocol for algorithmic interest'}
              </div>
            </div>
          ))}
