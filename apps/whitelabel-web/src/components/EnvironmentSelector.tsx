...
export function EnvironmentSelector() {
	...
	const environments: Array<{ key: "dev" | "prod"; label: string; icon: typeof Globe }> = [
		{ key: "dev", label: "Development", icon: Flame },
		{ key: "prod", label: "Production", icon: Globe },
	]
	...
	return (
		<DropdownMenu>
			<DropdownMenuTrigger asChild>
				<Button
					...
					className={cn(
						...
						environment === "dev" ? "border-orange-500 bg-orange-50 text-orange-700" : "border-green-600 bg-green-50 text-green-700",
					)}
				>
			...
			<DropdownMenuContent align="end" className="w-64">
				...
				<DropdownMenuSeparator />
				{environments.map(({ key, label, icon: Icon }) => {
					...
					return (
						<DropdownMenuItem
							key={key}
							onClick={() => enabled && setEnvironment(key)}
							disabled={!enabled}
