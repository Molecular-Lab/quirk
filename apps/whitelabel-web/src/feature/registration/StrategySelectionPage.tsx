...
export default function StrategySelectionPage() {
	...
	const [chatMessages] = useState([
		{
			role: 'assistant',
			content: "Hi! I'm your AI advisor. I've analyzed different revenue packages for you. Package 1 is recommended for balanced growth with conservative risk.",
		},
	])
			...
			<div className="max-w-[1400px] mx-auto px-6 py-8">
				...
				<div className="mb-6">
					...
					<p className="text-gray-600 text-sm">
						Selected: {selectedPackage.name} • Pure APY: {selectedPackage.pureAPY}% • You Get: {selectedPackage.clientTakes}% • End-User Gets: {selectedPackage.endUserGets}%
					</p>
				</div>
				...
				<div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
					{strategies.map((strategy) => (
						<div key={strategy.id} className="bg-gray-50 rounded-3xl p-6 flex flex-col items-center justify-center text-center min-h-[200px]">
							<h3 className="text-[20px] font-bold text-gray-900 mb-2">
								{strategy.name}
							</h3>
							<div className="text-[48px] font-bold text-primary-500 mb-2">
								{strategy.allocation}%
							</div>
				...
				{/* Continue Button */}
				<button
					className="w-full py-4 rounded-full font-semibold text-white bg-primary-500 hover:bg-primary-600 transition-all"
				>
