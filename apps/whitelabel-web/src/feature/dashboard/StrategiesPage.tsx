...
export function StrategiesPage() {
	...
	const [chatMessages] = useState([
		...
		{
			role: 'assistant',
			content: "I've analyzed different revenue packages for you. Mercury is recommended for balanced growth with conservative risk.",
			timestamp: '10:30 AM',
		},
	])
	...
	const updateAllocation = (id: string, value: string) => {
		...
		setStrategies((prev) =>
			prev.map((s) => (s.id === id ? { ...s, allocation: constrainedValue } : s))
		)
	}
							...
							<div className="grid grid-cols-3 gap-4 mb-6">
								{PACKAGES.map((pkg) => (
									...
									>
										...
										<div className="text-sm text-gray-600 mb-3">Pure APY: <span className="font-semibold text-gray-900">{pkg.pureAPY}%</span></div>
										<div className="flex items-center gap-2">
											<span className={`px-3 py-1 rounded-full text-xs font-medium ${
												pkg.id === 1 ? 'bg-gray-100 text-gray-700' :
												pkg.id === 2 ? 'bg-gray-200 text-gray-800' :
								...
								<div className="flex items-center justify-between">
									<div>
										...
										<p className="text-xs text-gray-600">
											Pure APY: {selectedPackage.pureAPY}% • Your Revenue: {selectedPackage.clientTakes}% • End-User Gets: {selectedPackage.endUserGets}%
										</p>
									</div>
								</div>
							</div>
							...
							<div key={strategy.id} className="bg-white border border-gray-200 rounded-3xl p-6">
								<div className="flex items-center justify-between mb-8">
									<div className="flex items-center gap-2">
										...
										<button
											onMouseEnter={() => setShowTooltip(strategy.id)}
