...
export const useUnwrapMutation = () => {
	...
	return useChainTxMutation({
		mutationFn: async (params: UnwrapParams): Promise<UnwrapResult> => {
			...
			if (walletClient.chain?.id !== chainId) {
				throw new Error(`[Unwrap] ChainId is not correct current:${walletClient.chain?.id} expected:${chainId}`)
			}
			if (!walletClient.account) {
				throw new Error("[Unwrap] walletClient is not connected to an account")
			}
