...
export class SafeAppProvider extends EventEmitter implements EIP1193Provider {
	...
	async request(request: { method: string; params?: unknown }): Promise<any> {
		...
		switch (method) {
			...
			case "wallet_requestPermissions": {
				const permissions = await this.sdk.wallet.requestPermissions(params[0])
				return permissions
			}
			...
			case "wallet_sendCalls": {
				...
				const txs = params[0].calls.map((call: { to?: Address; data?: Hex; value?: Hex; chainId?: Hex }, i: number) => {
			...
			case "wallet_getCallsStatus": {
				const CallStatus: Record<TransactionStatus, "PENDING" | "CONFIRMED"> = {
	...
	send(request: any, callback: (error: any, response?: any) => void): void {
		...
		this.request(request)
			.then((result) => {
				callback(null, { jsonrpc: "2.0", id: request.id, result: result })
			})
			.catch((error: unknown) => {
				callback(error, null)
			})
